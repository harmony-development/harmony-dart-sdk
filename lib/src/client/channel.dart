  ...
  Stream<Message> getMessages(Message before) async* {
    ...
    for (var response in responses) {
      yield Message(
          ...
          DateTime.fromMillisecondsSinceEpoch(int.parse(response["created_at"]),
              isUtc: true),
          response.containsKey("edited_at")
              ? DateTime.fromMillisecondsSinceEpoch(int.tryParse("edited_at"),
                  isUtc: true)
              : null,
          response["content"],
          response.containsKey("embeds")
              ? (response["embeds"] as List<Map<String, dynamic>>)
                  ?.map((element) => Embed.fromJson(element))
              : null,
          response.containsKey("actions")
              ? (response["actions"] as List<Map<String, dynamic>>)
                  ?.map((element) => Action.fromJson(element))
              : null);
    }
  }
}
