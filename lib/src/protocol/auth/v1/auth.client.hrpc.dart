...
class AuthServiceClient {
	...
	Future<FederateResponse> Federate(FederateRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc Federate error: ${response.statusCode}"); throw response; }
	...
	Future<LoginFederatedResponse> LoginFederated(LoginFederatedRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc LoginFederated error: ${response.statusCode}"); throw response; }
	...
	Future<KeyResponse> Key(KeyRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc Key error: ${response.statusCode}"); throw response; }
	...
	Future<BeginAuthResponse> BeginAuth(BeginAuthRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc BeginAuth error: ${response.statusCode}"); throw response; }
	...
	Future<NextStepResponse> NextStep(NextStepRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc NextStep error: ${response.statusCode}"); throw response; }
	...
	Future<StepBackResponse> StepBack(StepBackRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc StepBack error: ${response.statusCode}"); throw response; }
	...
	Stream<StreamStepsResponse> StreamSteps(StreamStepsRequest input, {Map<String,dynamic> headers = const {}}) async* {
		var socket = await $io.WebSocket.connect(this.server.replace(path: "/protocol.auth.v1.AuthService/StreamSteps").toString(), headers: {}..addAll(headers)..addAll(this.commonHeaders));
	...
	Future<CheckLoggedInResponse> CheckLoggedIn(CheckLoggedInRequest input, {Map<String,String> headers = const {}}) async {
		...
		if (response.statusCode != 200) { print("hrpc CheckLoggedIn error: ${response.statusCode}"); throw response; }
